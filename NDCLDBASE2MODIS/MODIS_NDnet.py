...
def stand_by_cell(ds, invert = False, var =  'N'): 
    ...
    ds_std =   dst[var+'_std'] 
    #stdmean =  ds_std.mean()
    #ds_std = ds_std.where(ds_std > 1e-6)
    #ds_std = ds_std.fillna(stdmean)
...
def get_data(var_out = 'MOD_CDNC_TAU5_Ztop32_CF80', pth_out = '', var_in = 'NCPL_CLDBASE', pth_in = ''):

    chk  = {"time": 1}
    print(pth_out)
    Nout  =  xr.open_mfdataset(pth_out, parallel=True, chunks=chk)[var_out]  
    print(Nout)     
    Nin  =  xr.open_mfdataset(pth_in, parallel=True, chunks=chk)[var_in]
    ...
    Nin = stand_by_cell(Nin, var =  var_in)
...
if __name__ == '__main__':
   #client = Client()
    ...
    pth_in= f"../data/aci_tavg_1dy_glo_L720x361_sfc/GiOCEAN_e1.aci_tavg_1dy_glo_L720x361_sfc.tavg1D."
    var_in = 'NCPL_CLDBASE' 
    ...
    if do_test:
        ...
        X_in = stand_by_cell(X, var =  var_in, invert =  True)*1e-6
        
 
        Npre = nb(amean(pred_xr).mean().load())
        Ntrue = nb(amean(Y_true).mean().load())
        Nin = nb(amean(X_in).mean().load()) 
        #restrict for Y_true > 1
        
        #pred_xr = pred_xr.where(Y_true > 1)
        #Y_true = Y_true.where(Y_true > 1) 
        ...
        cb0.set_label(r'Nd_cldbase (cm$^{-3}$)', fontsize=lbsz)
